...
class MessagesDatabase {
    ...
    public function addMessage(string $clan, string $sender, string $message): void {
        ...
        // Manter apenas as 15 mais recentes (usando subconsulta com prepared statement)
        $deleteStmt = $this->db->prepare("DELETE FROM messages WHERE id NOT IN (
            SELECT id FROM messages WHERE clan = :clan ORDER BY timestamp DESC LIMIT 15
        )");
        $deleteStmt->bindValue(":clan", $clan, SQLITE3_TEXT);
        $deleteStmt->execute();
    }
    ...
    public function deleteClanMessages(string $clan): void {
        $stmt = $this->db->prepare("DELETE FROM messages WHERE clan = :clan");
        $stmt->bindValue(":clan", $clan, SQLITE3_TEXT);
        $stmt->execute();
    }
